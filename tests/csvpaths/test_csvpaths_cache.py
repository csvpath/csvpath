...
class TestCsvPathsCache(unittest.TestCase):
    def test_cache_files(self):
        ...
        print(
            f"use cache config: {paths.config.get(section='cache', name='use_cache')}"
        )
        ...
        print(
            f"cache dir used: {os.getcwd()}/{paths.file_manager.lines_and_headers_cacher.cache._cachedir()}"
        )
        assert (
