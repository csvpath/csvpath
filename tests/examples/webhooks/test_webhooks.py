...
class TestWebhooks(unittest.TestCase):
    def test_webhooks_1(self):
        try:
            ...
            paths.file_manager.add_named_file(
                name="hooks",
                path=f"tests{os.sep}examples{os.sep}webhooks{os.sep}csvs{os.sep}hooks.csv",
            )
            paths.paths_manager.add_named_paths_from_json(
                file_path=f"tests{os.sep}examples{os.sep}webhooks{os.sep}csvpaths{os.sep}hooks.json"
            )
    ...
    def test_store_webhooks_for_paths(self):
        ...
        paths.paths_manager.add_named_paths_from_dir(
            name="hooks2",
            directory=f"tests{os.sep}examples{os.sep}webhooks{os.sep}csvpaths",
        )
