...
class TestBinaryCopy:
    def test_binary_copy_1(self):
        the_path = f"tests{os.sep}test_resources{os.sep}xlsx{os.sep}Table_1.1_Primary_Energy_Overview.xlsx"
        new_path = f"tests{os.sep}examples/binary_copy{os.sep}/temp.xlsx"
        nos = Nos(new_path)
        if nos.exists():
            nos.remove()
        if nos.exists():
